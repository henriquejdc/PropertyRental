...
class BaseAPITestCase(APITestCase):
    ...
    list_test_scenarios: List[str] = []
    retrieve_test_fields: List[str] = []
    ...
    def post(self, url: str, data: Dict[str, Any]) -> Response:
        response = self.client.post(url, data=data, format=self.request_format)
        return response
    ...
    def patch(self, url: str, data: Dict[str, Any]) -> Response:
        response = self.client.patch(
            url, data=data, format=self.request_format
        )
        return response
    ...
    def create_validation_error(self):
        ...
        """
        data = copy.copy(self.post_data)
        data[self.validation_error_column] = (
            None if self.request_format == "json" else ""
        )
        response = self.post(self.url, data)
        content = json.loads(response.content)
