...
class PropertyFilter(django_filters.FilterSet):
    ...
    address_neighborhood = django_filters.CharFilter(
        field_name="address_neighborhood", lookup_expr="icontains"
    )
    address_city = django_filters.CharFilter(
        field_name="address_city", lookup_expr="icontains"
    )
    address_state = django_filters.CharFilter(
...
class ReservationFilter(django_filters.FilterSet):
    ...
    host_id = django_filters.NumberFilter(
        field_name="property__host__id", lookup_expr="exact"
    )
    owner_id = django_filters.NumberFilter(
        field_name="property__owner__id", lookup_expr="exact"
    )
