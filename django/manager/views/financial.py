...
class CommissionViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                "type",
                openapi.IN_QUERY,
                description="Type 'seazone', 'owner', 'host",
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not commission_type or commission_type not in [
            "seazone",
            "owner",
            "host",
        ]:
        ...
        if (year and not month) or (month and not year):
            raise ValidationError(_("Required year and month."))
        ...
        total_commission = (
            commissions.aggregate(total_commission=Sum("commission_value"))[
                "total_commission"
            ]
            or 0
        )
        total_reservations = (
            commissions.aggregate(total_reservations=Count("id"))[
                "total_reservations"
            ]
            or 0
        )
        ...
        for property in Property.objects.all():
            property_commissions = commissions.filter(
                reservation__property=property
            )
            total_property_commission = (
                property_commissions.aggregate(
                    total_commission=Sum("commission_value")
                )["total_commission"]
                or 0
            )
            total_property_reservations = (
                property_commissions.aggregate(total_reservations=Count("id"))[
                    "total_reservations"
                ]
                or 0
            )
