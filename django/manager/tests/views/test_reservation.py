...
class ReservationViewSetTestCase(BaseAPITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.row_object: Reservation = baker.make(
            "manager.Reservation",
            client_name="Item1",
        ...
        self.row_object_two: Reservation = baker.make(
            "manager.Reservation",
            client_name="Item2",
        ...
        self.post_data = {
            "property": self.property.pk,  # type: ignore
            "client_name": "Item3",
            "client_email": "Item3@example.com",
            "start_date": (self.today + timedelta(days=2)).strftime(
                "%Y-%m-%d"
            ),
        ...
        self.total_rows = 2
        self.http_404_error_description = (
            "No Reservation matches the given query."
        )
    ...
    def test_reservation_interval_date_invalid(self):
        ...
        data.update(
            {
                "start_date": (self.today + timedelta(days=7)).strftime(
                    "%Y-%m-%d"
                ),
                "end_date": (self.today + timedelta(days=2)).strftime(
                    "%Y-%m-%d"
                ),
            }
        )
    ...
    def test_reservation_capacity_invalid(self):
        data = deepcopy(self.post_data)
        data.update({"guests_quantity": 1000})
        response = self.post(self.url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        contents = json.loads(response.content)
        self.assertEqual(
